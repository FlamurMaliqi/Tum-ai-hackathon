...
Combines:
- Claude streaming (from teammate's voice-agent branch)
- ElevenLabs TTS (text-to-speech audio streaming)

Flow: User text → Turn buffer → Claude (streaming) → TTS → Audio + Text to client
...
- {"type": "assistant_token", "text": "..."}   # streamed text chunks
- <binary frames>                              # streamed audio chunks (MP3)
- {"type": "assistant_done"}
...
async def handle_websocket(ws: WebSocket) -> None:
    ...
    async def _stream_assistant_response(user_text: str) -> None:
        """
        Stream Claude tokens through TTS to produce audio.
        
        Sends both text chunks (for display) and audio chunks (for playback).
        """
        ...
        try:
            # Create an async generator that yields Claude's text AND sends to client
            ...
            # Pipe Claude's text through TTS and send audio chunks
            ...
            logger.exception("Claude service error")
            await ws.send_json({"type": "assistant_error", "message": "anthropic_stream_error"})
        except Exception:
            logger.exception("Stream error")
            await ws.send_json({"type": "assistant_error", "message": "stream_error"})
        else:
    ...
    async def _flush_turn(trigger: str) -> None:
        ...
        # Persist the user's turn as a single message
    ...
    async def _append_to_turn(user_text: str) -> bool:
        ...
        Returns True if appended successfully, False if rejected.
        """
    ...
    try:
        ...
        while True:
            ...
            # Binary frames (placeholder for future audio input)
